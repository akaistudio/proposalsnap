...
def handle_500(e):
    ...
    return f"<pre style='padding:20px;background:#1a1a2e;color:#ff6b6b;font-size:12px'><b>500 Error</b>\n\n{tb}</pre>", 500
...
<a href="/welcome" style="text-decoration:none;color:inherit"><h1 class="app-logo">Proposal<span>Snap</span></h1></a>
...
def demo_login():
    ...
    if not user:
        cur.execute("""INSERT INTO users (email, password_hash, company_name, currency, is_superadmin)
                       VALUES (%s,%s,'Bloom Studio','INR',TRUE) RETURNING id""",
